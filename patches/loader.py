...
def load_inventory(path: str | Path) -> tuple["ClusterConfig", "TrainingConfig"]:
    ...
    for node_id, node_data in nodes_section.items():
        ...
        # Respect an explicit neighbour list (topology experiments: ring/star/
        # expander set this in the inventory). Fall back to full mesh otherwise,
        # so default runs are unchanged.
        if not node_data.get("neighbors"):
