...
from decentr_my_own.comm.config_server import (
    fetch_sync_participants,
    get_current_server,
    post_sync_ready,
)
...
class SyncRunOverrides:
    ...
    # Minimum fraction of neighbors required to proceed (soft barrier).
    min_quorum_fraction: float = 0.7
    # When True (bootstrap only): start the HTTP config server so followers can
    # fetch the run config, AND run decide_participants for soft-barrier consensus.
    serve_config: bool = False
...
def run_sync_worker(
    ...
    # Bootstrap only: start the HTTP config server so followers can fetch run config.
    # This also sets the _current_server singleton used by _soft_barrier_and_coordinate
    # to trigger decide_participants consensus.
    ...
    if overrides.serve_config:
        ...
        config_http_server.start()  # also sets _current_server singleton

    all_neighbors = [resolved.cluster.get_node(node_id) for node_id in resolved.self_node.neighbors]
    bootstrap_node = resolved.cluster.get_node(resolved.cluster.bootstrap_node_id)
    bootstrap_config_port = bootstrap_node.port + 1
    adaptive_runtime = None
    static_shard_ids: list[str] | None = None
    try:
        neighbors = _soft_barrier_and_coordinate(
            self_node_id=resolved.self_node_id,
            all_neighbors=all_neighbors,
            bootstrap_host=bootstrap_node.host,
            bootstrap_config_port=bootstrap_config_port,
            timeout_s=overrides.transport_timeout_s,
            min_quorum_fraction=overrides.min_quorum_fraction,
        )
        if not neighbors:
            raise RuntimeError(f"Node '{resolved.self_node_id}': no neighbors agreed, aborting.")
        if (
        ...
        for round_idx in range(overrides.rounds):
            ...
            # Push to each active neighbor; exclude any that fail (crashed mid-run).
            ...
            for neighbor in list(neighbors):
                ...
                try:
                    ...
                    _log.warning(
                        "Node '%s': push to %s failed at round %d (%s) — excluding",
                        resolved.self_node_id, neighbor.id, round_idx, exc,
                    )
                    dead_this_round.add(neighbor.id)
                finally:
                    client.close()

            active_neighbors = [n for n in neighbors if n.id not in dead_this_round]
            if not active_neighbors:
                raise RuntimeError(
                    f"Node '{resolved.self_node_id}': all neighbors failed at round {round_idx}, aborting"
                )
            received = server.wait_for_payload_ids(
                sender_node_ids=[n.id for n in active_neighbors],
                payload_id=payload_id,
                timeout_s=overrides.transport_timeout_s,
            )
            if not received:
                # Identify which neighbors never sent their payload using get_payload.
                missing = {
                    n.id for n in active_neighbors
                    if server.get_payload(n.id, payload_id=payload_id) is None
                }
                if missing:
                    _log.warning(
                        "Node '%s': round %d — neighbors %s did not send payload, excluding",
                        resolved.self_node_id, round_idx, sorted(missing),
                    )
                    dead_this_round |= missing
                    active_neighbors = [n for n in active_neighbors if n.id not in missing]
                    # Symmetric with the guard at the top of the round: if every
                    # remaining neighbor timed out (not just push-failed), abort
                    # instead of silently training solo. Without this, peer_payloads
                    # becomes [] and `any(... for x in [])` is vacuously False, so the
                    # round "succeeds" with no averaging and the node degrades quietly.
                    if not active_neighbors:
                        raise RuntimeError(
                            f"Node '{resolved.self_node_id}': all neighbors timed out at round {round_idx}, aborting"
                        )
                else:
                    # All payloads arrived but wait returned False — unexpected, raise.
                    raise TimeoutError(
                        f"Node '{resolved.self_node_id}' timed out waiting for neighbors at round {round_idx}"
                    )

            # Permanently drop neighbors that failed this round.
            if dead_this_round:
                neighbors = [n for n in neighbors if n.id not in dead_this_round]
                _log.warning(
                    "Node '%s': active neighbors after round %d: %s",
                    resolved.self_node_id, round_idx, [n.id for n in neighbors],
                )

            peer_payloads = [
                server.get_payload(neighbor.id, payload_id=payload_id) for neighbor in active_neighbors
            ]
...
def _soft_barrier_and_coordinate(
    self_node_id: str,
    all_neighbors: list,
    bootstrap_host: str,
    bootstrap_config_port: int,
    timeout_s: float,
    min_quorum_fraction: float = 0.7,
) -> list:
    """
    Soft barrier: probe neighbors, coordinate with bootstrap on who's reachable,
    return the agreed participant set. Never raises on individual node failures —
    only raises if we cannot reach a quorum.

    Flow:
      Phase 1 — quick probe  (up to probe_s):  ping all neighbors in parallel
      Phase 2 — report       (~5s):            POST /sync/ready with reachable set
      Phase 3 — coordinate   (up to coord_s):  if bootstrap, run decide_participants
                                                in background; everyone polls GET
      Phase 4 — confirm      (remaining time): ping agreed set until all respond
    """
    probe_s = min(timeout_s * 0.15, 45.0)
    coord_s = min(timeout_s * 0.25, 90.0)

    # ── Phase 1: quick probe ────────────────────────────────────────────────
    probe_deadline = time.time() + probe_s
    pending = {n.id: n for n in all_neighbors}
    reachable_ids: set[str] = set()

    while pending and time.time() < probe_deadline:
        for nid in list(pending):
            n = pending[nid]
            client = PeerClient(_node_target(n.host, n.port))
            try:
    ...
    _log.info(
        "Node '%s': probe done — reachable %d/%d: %s%s",
        self_node_id, len(reachable_ids), len(all_neighbors),
        sorted(reachable_ids)[:8],
        "…" if len(reachable_ids) > 8 else "",
    )

    # ── Phase 2: report to bootstrap ───────────────────────────────────────
    try:
        post_sync_ready(
            bootstrap_host, bootstrap_config_port,
            self_node_id, list(reachable_ids),
            timeout_s=10.0,
        )
    except Exception as exc:
        _log.warning("Node '%s': could not POST /sync/ready: %s", self_node_id, exc)

    # ── Phase 3: if we are bootstrap, decide participants in background ─────
    config_srv = get_current_server()
    if config_srv is not None:
        all_expected = [n.id for n in all_neighbors] + [self_node_id]
        threading.Thread(
            target=config_srv.decide_participants,
            kwargs=dict(
                expected_node_ids=all_expected,
                min_fraction=min_quorum_fraction,
                timeout_s=coord_s,
            ),
            daemon=True,
            name="decide-participants",
        ).start()

    # ── Phase 4: get agreed set from bootstrap ──────────────────────────────
    # Poll for coord_s + 60s to guarantee bootstrap finishes decide_participants
    # (which runs for coord_s) before we give up. Without this buffer we'd race.
    fetch_timeout_s = coord_s + 60.0
    try:
        agreed_ids = fetch_sync_participants(
            bootstrap_host, bootstrap_config_port,
            timeout_s=fetch_timeout_s,
        )
    except Exception as exc:
        _log.warning(
            "Node '%s': could not fetch agreed participants (%s) — falling back to reachable set",
            self_node_id, exc,
        )
        agreed_ids = list(reachable_ids)

    # Remove self from agreed_ids (we only want neighbors, not ourselves)
    agreed_neighbor_ids = [nid for nid in agreed_ids if nid != self_node_id]
    agreed_neighbors = [n for n in all_neighbors if n.id in agreed_neighbor_ids]

    min_required = max(1, int(len(all_neighbors) * min_quorum_fraction))
    if len(agreed_neighbors) < min_required:
        raise RuntimeError(
            f"Node '{self_node_id}': only {len(agreed_neighbors)}/{len(all_neighbors)} "
            f"neighbors agreed (need ≥{min_required})"
        )

    if len(agreed_neighbors) < len(all_neighbors):
        excluded = set(n.id for n in all_neighbors) - set(n.id for n in agreed_neighbors)
        _log.warning(
            "Node '%s': excluded %d nodes: %s",
            self_node_id, len(excluded), sorted(excluded),
        )

    # ── Phase 5: final confirm — wait for agreed neighbors to be pingable ───
    remaining = max(timeout_s - probe_s - coord_s - 10, 30.0)
    confirm_deadline = time.time() + remaining
    pending_confirm = {n.id: n for n in agreed_neighbors}
    while pending_confirm and time.time() < confirm_deadline:
        for nid in list(pending_confirm):
            n = pending_confirm[nid]
            client = PeerClient(_node_target(n.host, n.port))
            try:
                client.ping(sender_node_id=self_node_id, timeout_s=1.0)
                del pending_confirm[nid]
            except Exception:
                pass
            finally:
                client.close()
    ...
    if pending_confirm:
        _log.warning(
            "Node '%s': agreed neighbors still not pingable after confirm: %s — dropping them",
            self_node_id, list(pending_confirm),
        )
        agreed_neighbors = [n for n in agreed_neighbors if n.id not in pending_confirm]

    return agreed_neighbors
