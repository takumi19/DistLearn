...
# Peer-selection policy for async gossip push targets. "full" (default) keeps the
# original rotation behaviour, so existing experiments are unaffected. The other
# policies rank neighbours via a PeerScoreTracker (see algorithms/communication_policy.py).
CommunicationPolicy = Literal[
    "full",
    "random_fanout",
    "top_k_fastest",
    "top_k_reliable",
...
class AsyncConfig(BaseModel):
    ...
    # ── Peer-selection policy (Phase 2: CommunicationPolicy) ──────────────────
    # Default "full" == original behaviour. top_k_* / ping_aware / reliability_aware
    # use peer scores tracked during the run. push_fanout bounds how many targets
    # the score policies push to per step (0 = all, which makes top-k a no-op).
    communication_policy: CommunicationPolicy = "full"
    # EMA smoothing for the peer-score tracker (0..1; higher = react faster).
    peer_score_alpha: float = Field(default=0.3, gt=0, le=1)
    # ── Dynamic graph (Phase 3) ───────────────────────────────────────────────
    # 0 = disabled. When policy="dynamic", rebuild the active neighbour set every
    # N windows by ranking the full static-graph pool by ping_aware score.
    dynamic_graph_rebuild_windows: int = Field(default=0, ge=0)
    dynamic_graph_min_degree: int = Field(default=2, ge=1)
    # ── Delta exchange (Phase 3) ──────────────────────────────────────────────
    payload_mode: Literal["full", "delta"] = "full"
    # ── Compression (Phase 3) ────────────────────────────────────────────────
    compression: Literal["none", "float16", "quant8", "topk_sparse"] = "none"
    topk_fraction: float = Field(default=0.1, gt=0, lt=1)
