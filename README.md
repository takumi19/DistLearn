...
[График традиционного нераспределенного обучения](static/trad_run_results.png) показывает динамику лосса на обучении и валидации, а также точность и F1-score на тестовой выборке. Модель демонстрирует стабильное снижение лосса и рост метрик качества, достигая accuracy ~0.93 и F1 ~0.93 к 15-й эпохе.

В распределенном подходе с параметр-сервером анализировалась распределение batch loss по эпохам среди всех воркеров. [Боксплот](static/boxplot.jpeg) показывает уменьшение медианного loss с эпохами, с уменьшением дисперсии, что указывает на сходимость модели в распределенном подходе. Однако можно видеть выбросы на ранних эпохах, возможно, из-за асинхронных обновлений или неоднородности данных.
